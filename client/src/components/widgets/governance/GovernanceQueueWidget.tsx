...
function GovernanceItemCard({ item, mode }: { item: GovernanceItem; mode: DataMode }) {
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">{item.title || item.project || item.requirement || 'Governance Item'}</span>
          </div>
          <Badge variant={getStatusColor(item.status)}>{item.status}</Badge>
        </div>
        <div className="flex items-center gap-4 text-xs text-gray-500 ml-6">
          <span className={`px-2 py-0.5 rounded ${getPriorityColor(item.priority || 'medium')}`}>{item.priority || 'medium'}</span>
            ...
            <div className="p-4 bg-gray-50">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-white p-3 rounded-lg border">
                  ...
                  <p className="font-semibold text-sm capitalize">{item.type || 'Review'}</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
              ...
              <div className="bg-white p-3 rounded-lg border mb-4">
                ...
                <p className="font-semibold text-sm">{item.relatedRisks || 0} high-severity risks in category</p>
              </div>
              ...
              <div className={`p-3 rounded-lg border ${mode === 'VRO' ? 'bg-purple-50 border-purple-100' : 'bg-gray-100 border-gray-200'}`}>
                ...
                <p className="text-sm text-gray-700">{item.aiStatus || 'No AI status available'}</p>
              </div>
            </div>
          </motion.div>
        )}
    ...
    <Card className="h-full">
      ...
      <CardContent>
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {(governanceItems as GovernanceItem[]).map((item: GovernanceItem, i: number) => (
