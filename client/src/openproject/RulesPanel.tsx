...
/**
 * An ML-suggested threshold for a rule, learned from the HITL learning loop
 * (approved breach = true positive, rejected = false positive). Advisory only —
 * authoring stays in OpenProject. Served by GET /api/agent/rules/suggestions.
 */
...
export function RulesPanel({
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const [rulesRes, breachRes, suggRes] = await Promise.all([
        fetch(`${apiBase}/rules`),
        fetch(`${apiBase}/findings?type=RuleBreach`).catch(() => null),
        fetch(`${apiBase}/rules/suggestions`).catch(() => null),
      ]);
      ...
      if (suggRes?.ok) {
        ...
        setSuggestions(Array.isArray(suggData) ? suggData : (suggData.suggestions ?? []));
      } else {
        setSuggestions([]);
      }
    } catch (err: any) {
  ...
  const suggestionByRuleId = useMemo(() => {
    const map = new Map<string, ThresholdSuggestion>();
    for (const s of suggestions) map.set(String(s.ruleId), s);
    return map;
  }, [suggestions]);
      ...
      <div className="flex flex-col gap-2">
        {sortedRules.map((rule) => (
          ...
          >
            ...
            {(() => {
              const sugg = suggestionByRuleId.get(String(rule.id));
              if (!sugg || sugg.direction === "keep") return null;
              return (
                <div className="mt-2 flex flex-wrap items-center gap-2 rounded-md border border-violet-500/30 bg-violet-500/5 px-2 py-1.5">
                  ...
                  <span className="text-[11px] text-neutral-700 dark:text-neutral-300">
                    {sugg.direction === "raise" ? "Raise" : "Lower"} threshold to{" "}
                    <span className="font-mono font-semibold">{sugg.suggestedThreshold}</span>{" "}
                    <span className="text-neutral-400">
                      ({sugg.confidence} confidence · n={sugg.sampleSize})
                    </span>
                  </span>
                  <span className="w-full text-[10px] text-neutral-500">{sugg.rationale}</span>
                </div>
              );
            })()}
          </div>
