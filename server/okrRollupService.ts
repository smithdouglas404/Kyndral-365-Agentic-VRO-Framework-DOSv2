 ...
 * entity aligns to an OKR; `okrEntityContributions` (see shared/schema.ts)
 * records *how much*. This service turns those rows into KR/OKR progress
 * numbers:
 *
 ...
 * query (see server/storage.ts). Decimal columns come back from Drizzle as
 * strings — all numeric inputs accept `number | string` and are coerced.
 *
 * KYNDRAL ADAPTATION: all ids are varchar UUIDs (string), matching
 * shared/schema.ts (okrs.id, keyResults.id, projects.id, tasks.id, …).
 * keyResults has no per-KR weight column, so KR weight defaults to 1
 * (equal-weighted average); okrs.progress and keyResults.progress are
 * integer columns, so the write-back rounds to whole percentages.
 */
...
export interface KeyResultRow {
  id: string;
  okrId: string;
  /** Relative weight within the OKR; defaults to 1 when null/absent. */
  weight?: NumberLike;
 ...
 *
 *   async getOkrEntityContributions(keyResultId: string) {
...
export interface RollupStorage {
  getKeyResults(okrId: string): Promise<KeyResultRow[]>;
  getContributions(keyResultId: string): Promise<ContributionRow[]>;
...
export class OkrRollupService {
   ...
   */
  async rollUpKeyResult(keyResultId: string): Promise<KeyResultRollup> {
   ...
   * (weight = keyResults.weight, defaulting to 1 when absent — Kyndral's
   * keyResults table has no weight column, so KRs are equal-weighted).
   */
  async rollUpOkr(okrId: string): Promise<OkrRollup> {
   ...
   * storage.upsertOkrEntityContribution — upsert on (keyResultId, entityType,
   * entityId).
   */
  inferContributions(okrId: string, candidates: LinkageCandidate[]): ContributionRow[] {
    // Group per key result (null = OKR-level linkage).
    const byKr = new Map<string | null, LinkageCandidate[]>();
...
/** Write surface for persisting rollups — implemented by Kyndral's storage
 * (`db.update(keyResults).set(patch).where(eq(...))`, see server/storage.ts). */
export interface RollupWriteStorage extends RollupStorage {
  updateKeyResult(
    id: string,
    patch: { progress?: number; currentValue?: number },
  ): Promise<unknown>;
  updateOkr(id: string, patch: { progress?: number }): Promise<unknown>;
}
 ...
 *
 * Kyndral columns written: keyResults.progress (integer 0–100, rounded),
 * keyResults.currentValue (real — progress applied to targetValue), and
 * okrs.progress (integer 0–100, rounded) — the columns the OKR dashboard reads.
 */
  ...
  for (const okrId of okrIds) {
    ...
    for (const kr of rollup.keyResults) {
      ...
      await storage.updateKeyResult(kr.keyResultId, {
        // kr.progress ← rolled 0–100 (integer column in Kyndral's schema)
    ...
    // okr.progress ← weighted KR average (integer column in Kyndral's schema).
    await storage.updateOkr(okrId, { progress: Math.round(rollup.progress) });
    results.push(rollup);
  }
