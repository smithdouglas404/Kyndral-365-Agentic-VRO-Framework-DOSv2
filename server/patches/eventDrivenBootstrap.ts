 ...
 */
// NOTE: in this tree the engine is ALREADY activated from server/index.ts via
// getEventDrivenOrchestrator(storage) + registerAgent + startListening(5000).
// activateEventDrivenOrchestration() below is kept for parity with the spec
// (and for tests/standalone use); prefer the index.ts singleton in app code.
import { EventDrivenOrchestrator, type ChangeEvent as EngineChangeEvent } from "../lib/EventDrivenOrchestrator.js";
...
export function registerCrudChangeHooks(orchestrator: EventDrivenOrchestrator): CrudChangeHooks {
  // Map the loose hook types onto the engine's ChangeEvent union.
  const TYPE_MAP: Record<string, EngineChangeEvent["type"]> = {
    budget: "budget_change",
    schedule: "schedule_change",
    risk: "risk_change",
    status: "project_update",
    scope: "project_update",
  };

  const emit = (event: ChangeEvent): void => {
    orchestrator.registerChange({
      type: TYPE_MAP[event.type] ?? "project_update",
      projectId: String(event.projectId ?? event.entityId ?? ""),
      field: event.summary,
      previousValue: event.prev,
      newValue: event.next,
      severity: (event.severity as EngineChangeEvent["severity"]) ?? "medium",
      timestamp: new Date(),
      source: event.source ?? "crud",
    });
  };
