...
/** Read lazily so the route degrades (503) rather than assuming a default. */
...
async function forward(
  ...
  if (!RUNTIME_URL) {
    res.status(503).json({
      error:
        "agent-runtime is not configured: set AGENT_RUNTIME_URL (and AGENT_RUNTIME_TOKEN if the runtime requires a bearer token)",
    });
    return;
  }
