...
export interface OpenProjectRoutesDeps {
  /**
   * The write-back service, or null when OpenProject is not configured
   * (no OPENPROJECT_BASE_URL/OPENPROJECT_URL + OPENPROJECT_API_KEY) — the
   * routes then respond 503 with a clear message instead of crashing.
   */
  writeback: OpenProjectWriteback | null;
...
export function initOpenProjectRoutes(router: Router, deps: OpenProjectRoutesDeps): Router {
  ...
  if (!writeback) {
    // OpenProject not configured — degrade gracefully instead of crashing.
    router.use("/api/openproject", (_req: Request, res: Response) => {
      res.status(503).json({
        ok: false,
        error:
          "OpenProject integration is not configured: set OPENPROJECT_BASE_URL (or OPENPROJECT_URL) and OPENPROJECT_API_KEY",
      });
    });
    return router;
  }
