...
import { agenticTools, AGENTIC_SYSTEM_PROMPT } from "../ai-sdk/tools.js";
...
export function initAgentChatRoute(router: Router): Router {
  router.post("/api/agent-chat", async (req: Request, res: Response) => {
    if (!process.env.ANTHROPIC_API_KEY) {
      res.status(503).json({
        error:
          "Agent chat is not configured: set ANTHROPIC_API_KEY (and optionally " +
          "AGENT_RUNTIME_URL for the agent-runtime sidecar) to enable it.",
      });
      return;
    }
