...
/**
 * Webhook-side echo check: does this OpenProject webhook payload correspond to
 * a write WE just made? True when the work-package / project id is in the
 * recentlyPushed registry (layer 1) or the payload's comment carries the
 * SYNC_MARKER (layer 2). Called by server/routes/webhooks/openproject.ts
 * alongside its existing customField_sync_source === 'nextera-agent' guard.
 */
export function isOwnEcho(payload: any): boolean {
  const wpId = payload?.work_package?.id;
  if (wpId !== undefined && wpId !== null && wasRecentlyPushed(wpId)) return true;
  const projectId = payload?.project?.id;
  if (projectId !== undefined && projectId !== null && wasRecentlyPushed(`project:${projectId}`)) {
    return true;
  }
  // Marker comments: activity/comment-shaped payloads carrying [sync:kyndral-365].
  const comment =
    payload?.activity?.comment?.raw ??
    payload?.comment?.raw ??
    payload?.work_package?._embedded?.activity?.comment?.raw;
  if (typeof comment === "string" && comment.includes(SYNC_MARKER)) return true;
  return false;
}
...
/**
 * Build the write-back service from environment variables.
 * OPENPROJECT_BASE_URL is preferred; OPENPROJECT_URL (used elsewhere in this
 * repo, e.g. MCPServiceFactory) is the fallback. Returns null when the
 * instance URL or OPENPROJECT_API_KEY is not configured.
 */
export function createOpenProjectWritebackFromEnv(): OpenProjectWriteback | null {
  ...
  return new OpenProjectWriteback({ baseUrl, apiKey });
}
